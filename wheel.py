...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self,guessed):
        ...
        if VOWEL_COST<=self.prizeMoney:
            for s in LETTERS:
                if s not in guessed:
                    can_be_guessed.append(s)
        else:
            for s in LETTERS:
                if (s not in guessed) and (s not in VOWELS):
                    can_be_guessed.append(s)
        return can_be_guessed
    def getMove(self, category, obscuredPhrase, guessed):
        possibleCharacters = self.getPossibleLetters(guessed)
        if possibleCharacters == []:
            return "pass"
        elif self.smartCoinFlip() == True:
            for letter in "ZQXJKVBPYGFWMUCLDRHSNIOATE"[::-1]:
                if letter in possibleCharacters:
                    return letter
        else:
